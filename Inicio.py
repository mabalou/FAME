...
st.markdown("""
<style>
[data-testid="stAppViewContainer"]{opacity:0;animation:fadeIn .3s ease-in forwards}
...
div.block-container{{padding-top:5.5rem!important}} /* ↓ mucho menor separación */

/* menú centrado */
.menu-links{{display:flex;gap:1.8rem;flex-wrap:wrap;align-items:center;justify-content:center;margin-bottom:.4rem}}
.menu-link{{font-size:1.05rem;font-weight:600;text-decoration:none!important;color:var(--menu-link)!important;border-bottom:2px solid transparent;transition:all .25s}}
.menu-link:hover,.menu-link.active{{color:var(--menu-active)!important;border-bottom:2px solid var(--menu-active)}}

/* botones especiales */
.menu-btn{{background:var(--menu-link);color:#fff!important;font-weight:600;padding:6px 16px;border-radius:25px;text-decoration:none!important;border:1px solid rgba(255,255,255,.15);transition:all .25s}}
...
.theme-toggle span,.lang-toggle span{{text-decoration:none!important}} /* 🔥 Refuerzo extra */
...
/* MÉTRICAS: asegurar color en claro/oscuro */
div[data-testid="stMetric"] div, 
div[data-testid="stMetricValue"], 
div[data-testid="stMetricLabel"]{{color:var(--metric-text)!important}}
</style>

<!-- CABECERA -->
<div class="header-bar">
  <div class="menu-links">
    <a class="menu-link {'active' if st.session_state.current_page=='Inicio' else ''}" href="?page=Inicio&theme={theme}&lang={lang}" target="_self">{TEXTS[lang]['inicio']}</a>
...
if logo.exists():
    ...
    <div style='text-align:center;margin-top:.2rem;'>
        <img src="data:image/png;base64,{img_b64}" width="260" style="border-radius:20px;"/>
